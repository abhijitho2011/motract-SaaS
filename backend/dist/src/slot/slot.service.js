...
let SlotService = class SlotService {
    ...
    async generateSlots(bayId, date, startStr, endStr, durationMin) {
        return [];
    }
    async getSlots(bayId, date) {
        const dateStr = date.toISOString();
        return this.db
            .select()
            .from(schema_1.slotBookings)
            .where((0, drizzle_orm_1.and)((0, drizzle_orm_1.eq)(schema_1.slotBookings.bayId, bayId), (0, drizzle_orm_1.eq)(schema_1.slotBookings.date, dateStr)));
    }
    async bookSlot(data) {
        const result = await this.db.insert(schema_1.slotBookings).values(data).returning();
        return result[0];
    }
};
